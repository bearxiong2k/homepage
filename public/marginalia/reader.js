...
function setReaderPageZoom(zoom, options = {}) {
  ...
  if (state.splitNotesActive) cancelSplitRemoteScrollFollower();
...
function setNotesPanelCollapsed(collapsed) {
  ...
  if (!collapsed) requestAnimationFrame(focusActiveNavigatorCard);
}
...
function handleSplitNotesMessage(envelope) {
  ...
  if (type === 'toggle-pin-note') {
    if (payload.annotationId) togglePinnedNote(payload.annotationId);
    return;
  }
  if (type === 'toggle-focus-note') {
    if (payload.annotationId) toggleFocusMode(payload.annotationId);
    return;
  }
  if (type === 'toggle-attach-highlight') {
    if (payload.annotationId) startAttachHighlightMode(payload.annotationId);
    return;
  }
  if (type === 'toggle-remove-highlight') {
    if (payload.annotationId) startRemoveHighlightMode(payload.annotationId);
    return;
  }
...
function closeSplitNotesSession(options = {}) {
  ...
  if (state.splitScrollRaf) {
    (state.splitScrollRafView || window).cancelAnimationFrame(state.splitScrollRaf);
    state.splitScrollRaf = 0;
    state.splitScrollRafView = null;
  }
...
function broadcastSplitSourceScroll(doc = getFrameDoc()) {
  ...
  const frameView = doc.defaultView;
  state.splitScrollRafView = frameView;
  state.splitScrollRaf = frameView.requestAnimationFrame(() => {
    state.splitScrollRaf = 0;
    state.splitScrollRafView = null;
    if (!state.splitNotesActive || !state.splitChannel || !doc?.defaultView) return;
...
function applySplitNotesScroll(scrollY, sentAt = 0) {
  ...
  state.splitPendingScrollY = y;
  if (splitScrollMotionReduced(view)) {
    cancelSplitRemoteScrollFollower();
    if (Math.abs((view.scrollY || 0) - y) <= SPLIT_SCROLL_SETTLE_DISTANCE) return;
...
function scheduleSplitRemoteScrollFollower(view = state.splitRemoteScrollView) {
  if (state.splitRemoteScrollRaf || state.splitRemoteScrollDestinationY == null || !view) return;
  state.splitRemoteScrollRaf = view.requestAnimationFrame((timestamp) => {
    state.splitRemoteScrollRaf = 0;
    stepSplitRemoteScrollFollower(view, timestamp);
  });
}

function stepSplitRemoteScrollFollower(view = state.splitRemoteScrollView, timestamp = null) {
  if (state.splitRemoteScrollDestinationY == null || !view) return;
  if (view !== getFrameDoc()?.defaultView) {
    cancelSplitRemoteScrollFollower();
    return;
  }
  const frameNow = Number.isFinite(Number(timestamp)) ? Number(timestamp) : view.performance.now();
  if (frameNow < state.splitRemoteScrollReadyAt) {
    scheduleSplitRemoteScrollFollower(view);
    return;
  }
  const scrollHeight = Math.max(
  ...
  const target = clampSplitScrollPosition(
    state.splitRemoteScrollDestinationY,
    scrollHeight,
    view.innerHeight
  );
  state.splitRemoteScrollDestinationY = target;
  const current = Math.max(0, view.scrollY || 0);
  const elapsedMs = state.splitRemoteScrollLastFrameAt
...
function cancelSplitRemoteScrollFollower() {
  const view = state.splitRemoteScrollView;
  if (state.splitRemoteScrollRaf) (view || window).cancelAnimationFrame(state.splitRemoteScrollRaf);
  state.splitRemoteScrollRaf = 0;
  state.splitRemoteScrollDestinationY = null;
...
function buildSplitNotesSourceState(doc = state.iframeLoaded ? getFrameDoc() : null) {
  ...
  return {
    ...
    pinnedAnnotationId: state.pinnedAnnotationId || null,
    focusModeAnnotationId: state.focusModeAnnotationId || null,
    collapsedSideNoteIds: [...state.collapsedSideNoteIds],
    mode: state.mode,
    attachTargetAnnotationId: state.attachTargetAnnotationId || null,
...
function beginInlineTextEdit(annotationId, note, focusField = 'body', pointerEvent = null, requestedBlockId = '') {
  ...
  let modeButton = null;
  let markdownEditAnchor = null;
  if (field === body) {
    const block = sideNoteBlockById(annotation, body.dataset.blockId);
    if (block?.type !== 'text') return;
    if (body.classList.contains('is-rendered')) {
      markdownEditAnchor = captureNoteMarkdownEditAnchor(body, pointerEvent, block.markdown);
    }
    body.textContent = block.markdown;
    body.classList.remove('is-rendered', 'note-markdown');
    body.tabIndex = 0;
...
function focusActiveNavigatorCard() {
...
function activateAnnotation(annotationId, scrollIntoView, jumpOptions = {}) {
  state.activeAnnotationId = annotationId;
  const annotation = state.annotations.find((item) => item.id === annotationId);
  if (!annotation) return;
  const doc = getFrameDoc();
  syncActiveAnnotationState();
  if (scrollIntoView) {
    jumpToAnnotation(annotationId, jumpOptions);
  }
  renderNoteList({ anchorAnnotationId: annotationId });
  syncJumpToNoteButton(doc);
}
...
function jumpToAnnotation(annotationId, options) {
  options = options || {};
  const doc = getFrameDoc();
  const escaped = cssEscape(annotationId);
  const note = doc.querySelector(`.reader-side-note[data-annotation-id="${escaped}"]`);
  const annotation = state.annotations.find((item) => item.id === annotationId);
  if (requestPdfPageForAnnotationJump(doc, annotation, options)) {
    syncJumpToNoteButton(doc);
    return;
  }
...
async function settlePendingPdfAnnotationJump(doc, pending) {
  let previousGeometry = null;
  let stableFrames = 0;
  for (let frame = 0; frame < PDF_NOTE_JUMP_SETTLE_MAX_FRAMES; frame += 1) {
    await nextPdfAnnotationJumpFrame(doc);
    if (state.pendingPdfAnnotationJump !== pending || doc !== getFrameDoc()) return;
    if (!pdfAnnotationJumpReady(doc, pending)) {
      pending.settling = false;
      return;
    }
    layoutSideNotes(doc);
    const geometry = pendingPdfAnnotationJumpGeometry(doc, pending);
    if (!geometry) {
      previousGeometry = null;
      stableFrames = 0;
      continue;
    }
    stableFrames = pdfAnnotationJumpGeometryMatches(previousGeometry, geometry)
      ? stableFrames + 1
      : 1;
    previousGeometry = geometry;
    if (stableFrames < PDF_NOTE_JUMP_SETTLE_STABLE_FRAMES) continue;
    completePendingPdfAnnotationJump(doc, pending, geometry);
    return;
  }
  pending.settling = false;
}

function nextPdfAnnotationJumpFrame(doc) {
  return new Promise((resolve) => {
    const requestFrame = doc?.defaultView?.requestAnimationFrame || window.requestAnimationFrame;
    requestFrame.call(doc?.defaultView || window, () => resolve());
  });
}

function pendingPdfAnnotationJumpGeometry(doc, pending) {
  const page = pdfAnnotationJumpPage(doc, pending);
  const annotation = state.annotations.find((item) => item.id === pending?.annotationId);
  const position = sideNotePosition(doc, annotation);
  const note = doc.querySelector(`.reader-side-note[data-annotation-id="${cssEscape(pending?.annotationId)}"]`);
  if (!page || !Number.isFinite(position?.top)) return null;
  const pageRect = page.getBoundingClientRect();
  const noteRect = note?.getBoundingClientRect?.();
  const scrollY = Math.max(0, Number(doc.defaultView?.scrollY) || 0);
  const scrollHeight = Math.max(
    Number(doc.documentElement?.scrollHeight) || 0,
    Number(doc.body?.scrollHeight) || 0
  );
  const geometry = {
    pageTop: scrollY + pageRect.top,
    pageHeight: pageRect.height,
    noteTop: position.top,
    noteHeight: Math.max(0, Number(noteRect?.height) || 0),
    scrollHeight
  };
  return Object.values(geometry).every(Number.isFinite)
    && geometry.pageHeight > 1
    ? geometry
    : null;
}

function pdfAnnotationJumpGeometryMatches(previous, current) {
  if (!previous || !current) return false;
  return ['pageTop', 'pageHeight', 'noteTop', 'noteHeight', 'scrollHeight']
    .every((key) => Math.abs(previous[key] - current[key]) <= PDF_NOTE_JUMP_GEOMETRY_EPSILON);
}

function completePendingPdfAnnotationJump(doc, pending, geometry) {
  if (state.pendingPdfAnnotationJump !== pending || doc !== getFrameDoc()) return false;
  state.pendingPdfAnnotationJump = null;
  state.pdfPendingJumpStatusUntil = 0;
  if (state.pdfPendingJumpNotice?.annotationId === pending.annotationId) state.pdfPendingJumpNotice = null;
  ...
  renderNavigatorNoteCards([pending.annotationId]);
  const viewportOffset = pending.alignNoteTop
    ? 0
    : doc.defaultView.innerHeight * NOTE_JUMP_VIEWPORT_OFFSET_RATIO;
  doc.defaultView.scrollTo(0, Math.max(0, geometry.noteTop - viewportOffset));
  syncJumpToNoteButton(doc);
  scheduleSplitNotesStateBroadcast(doc);
  setStatus('Moved to selected note.');
  return true;
}

function pdfAnnotationJumpPage(doc, pending) {
  if (!Number.isInteger(pending?.pageIndex) || pending.pageIndex < 0) return null;
  return doc.querySelector(`[data-pdf-page-index="${cssEscape(String(pending.pageIndex))}"]`);
}
